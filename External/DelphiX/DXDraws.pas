...
type
  TDrawMode=(dmNormal,dmAdd,dmSub);
...
{$ENDIF}
    procedure DrawAdd(const DestRect, SrcRect: TRect; Source: TDirectDrawSurface;
      Transparent: Boolean; Alpha: Integer{$IFDEF DX_Delphi4_Up}=255{$ENDIF});
    ...
    procedure DrawRotateAdd(X, Y, Width, Height: Integer; const SrcRect: TRect;
      Source: TDirectDrawSurface; CenterX, CenterY: Double; Transparent: Boolean; Angle: Integer;
      Alpha: Integer{$IFDEF DX_Delphi4_Up}=255{$ENDIF});
...
{$ENDIF}
  ...
  TDXDrawOption = (doFullScreen, doNoWindowChange, doAllowReboot, doWaitVBlank,
    ...
    do3D, doRetainedMode, doHardware, doSelectDriver, doZBuffer);
  ...
  protected
    ...
    {$ifdef DX_Delphi4_Up}procedure resize;{$endif}
  ...
  published
    {$IFDEF DX_Delphi4_Up}property Anchors;{$ENDIF}
    property AutoInitialize;
    property AutoSize;
    property Color;
    {$IFDEF DX_Delphi4_Up}property Constraints;{$ENDIF}
    property Display;
    property Options;
    ...
    property OnMouseUp;
    {$IFDEF DX_Delphi4_Up}property OnResize;{$ENDIF}
  ...
  public
    ...
    procedure DrawImage(Dest: TDirectDrawSurface;X,Y,Width,Height,PatternIndex:Integer;CenterX,CenterY,Angle:Double; Alpha: Integer;DrawMode:TDrawMode);
    procedure DrawImageAdd (Dest: TDirectDrawSurface;X,Y,Width,Height,PatternIndex:Integer;CenterX,CenterY,Angle:Double; Alpha: Integer);
    procedure DrawImageSub (Dest: TDirectDrawSurface;X,Y,Width,Height,PatternIndex:Integer;CenterX,CenterY,Angle:Double; Alpha: Integer);
    procedure DrawImageNorm(Dest: TDirectDrawSurface;X,Y,Width,Height,PatternIndex:Integer;CenterX,CenterY,Angle:Double; Alpha: Integer);
    procedure DrawWave(Dest: TDirectDrawSurface;X,Y,Width,Height,PatternIndex:Integer;Amp,Len,Pos, Alpha: Integer;DrawMode:TDrawMode);
    ...
    procedure DrawWaveXAdd(Dest: TDirectDrawSurface; X, Y, Width, Height: Integer; PatternIndex: Integer;
      amp, Len, ph: Integer; Alpha: Integer{$IFDEF DX_Delphi4_Up}=255{$ENDIF});
    ...
    property Width: Integer read GetWidth;
  published
    //property Image_Name : shortstring read FImage_Name write FImage_Name;
...
begin
  if FIDDSurface4=Value then Exit;
...
begin
  ...
  begin
    ...
    try
      DDraw.IDraw.EnumDisplayModes(0, PDDSurfaceDesc(nil)^,
        FModes, @EnumDisplayModesProc);
    finally
      DDraw.Free;
    end;
...
begin
  try
    ...
    with D3DDevice3 do
    begin
      SetRenderState(D3DRENDERSTATE_DITHERENABLE, 1);
...
{$ifdef DX_Delphi4_Up}
procedure TCustomDXDraw.resize;
begin
 if Autosize then SetSize(width,height);
 inherited;
end;
{$endif}
...
procedure TDXDrawDriver.Flip;
begin
end;

procedure TDXDrawDriver.Initialize;
begin
end;
...
begin
  ...
  Options := [doAllowReboot, doWaitVBlank, doCenter, doHardware, doRetainedMode, doSelectDriver];
      ...
      begin
        ...
        begin
          ...
          FSurface := TDirectDrawSurface(FSurfaceList[(x2 div BlockWidth)+
            ((y2 div BlockHeight)*BlockXCount)]);
        end;
      end;
  end;
end;
...
procedure TPictureCollectionItem.DrawWave(Dest: TDirectDrawSurface; X, Y,
  Width, Height, PatternIndex, Amp, Len, Pos, Alpha: Integer;
  DrawMode: TDrawMode);
begin
 case DrawMode of
   dmNormal:if alpha=255 then DrawWaveX     (Dest,x,y,width,height,Patternindex,amp,Len,pos)
                         else DrawWaveXAlpha(Dest,x,y,width,height,Patternindex,amp,Len,pos,alpha);
   dmAdd: DrawWaveXAdd(Dest,x,y,width,height,Patternindex,amp,Len,pos,alpha);
   dmSub: DrawWaveXSub(Dest,x,y,width,height,Patternindex,amp,Len,pos,alpha);
  end;
end;



procedure TPictureCollectionItem.DrawImageAdd(Dest: TDirectDrawSurface; X,
  Y, Width, Height, PatternIndex: Integer; CenterX, CenterY, Angle: Double;
  Alpha: Integer);
var
  r: TRect;
begin
  r := Bounds(x-Trunc(Centerx*width), y-Trunc(centery*height), Width, Height);

  if round(Angle*256) and $FF=0 then
  begin
    if Alpha<255 then
    begin
      DrawAdd(Dest, r, Patternindex, Alpha)
    end else
    begin
      DrawAdd(Dest, r, Patternindex);
    end;
  end else
  begin
    if Alpha<255 then
    begin
      DrawRotateAdd(Dest, X, Y,
        Width, Height, PatternIndex, CenterX, CenterY, round(Angle*256), Alpha)
    end else
    begin
      DrawRotateAdd(Dest, X,Y,
        Width, Height, PatternIndex, CenterX, CenterY, round(Angle*256))
    end;
  end;
end;

procedure TPictureCollectionItem.DrawImageNorm(Dest: TDirectDrawSurface; X,
  Y, Width, Height, PatternIndex: Integer; CenterX, CenterY, Angle: Double;
  Alpha: Integer);
var
  r: TRect;
begin
  ...
  begin
    if Alpha<255 then
    begin
      DrawAlpha(Dest, r, Patternindex, Alpha)
    end else
    begin
      StretchDraw(Dest, r, Patternindex);
    end;
  end else
  begin
    if Alpha<255 then
    begin
      DrawRotateAlpha(Dest, X, Y,
        Width, Height, PatternIndex, CenterX, CenterY, round(Angle*256), Alpha)
    end else
    begin
      DrawRotate(Dest, X,Y,
...
procedure TPictureCollectionItem.DrawImageSub(Dest: TDirectDrawSurface; X,
  Y, Width, Height, PatternIndex: Integer; CenterX, CenterY, Angle: Double;
  Alpha: Integer);
var
  r: TRect;
begin
  ...
  begin
    if Alpha<255 then
    begin
      DrawSub(Dest, r, Patternindex, Alpha)
    end else
    begin
      DrawSub(Dest, r, Patternindex);
    end;
  end else
  begin
    if Alpha<255 then
    begin
      DrawRotateSub(Dest, X, Y,
        Width, Height, PatternIndex, CenterX, CenterY, round(Angle*256), Alpha)
    end else
    begin
      DrawRotateSub(Dest, X,Y,
...
procedure TPictureCollectionItem.DrawImage(Dest: TDirectDrawSurface; X, Y,
  Width, Height, PatternIndex: Integer; CenterX, CenterY, Angle: Double;
  Alpha: Integer; DrawMode: TDrawMode);
begin
 case DrawMode of
   dmNormal:DrawImageNorm(Dest,x,y,width,height,patternindex,centerx,centery,angle,alpha);
   dmAdd   :DrawImageAdd (Dest,x,y,width,height,patternindex,centerx,centery,angle,alpha);
   dmSub   :DrawImageSub (Dest,x,y,width,height,patternindex,centerx,centery,angle,alpha);
  end;
end;
