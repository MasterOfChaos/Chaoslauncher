...
var SelectionCirclesPatches:TPatchGroup;
    ...
    CircleNeededUnknown:TManualStringPatch;
...
begin
  ...
  try
    ...
    CircleNeededUnknown:=TManualStringPatch.create(SelectionCirclesPatches,CircleNeededAddr+$3F,#$EB);//JMP instead of JE
    ...
    GetTeamCount:=TManualStringPatch.create(SelectionCirclesPatches,GetTeamCountAddr,GetTeamCounStr);
...
begin
  try
    if not SelectionCirclesPatches.Enabled
      then begin//disabled
        if not settings.SelectionCircles then exit;
        //Replay=>enable
...
initialization
  {AddInitHandler(CirclesInit,[pmInjected]);
  AddFinishHandler(CirclesFinish,[pmInjected]);
  AddTimerHandler(CirclesTimer,[pmInjected]); }
end.
